...
public class GraczX extends Gracz {
    ...
    public void mojRuch(Plansza plansza, int size) {
        ...
        while (true) {
            System.out.println(this.name + "_" + Symbol.X.getS() + " podaj swój ruch (wiersz/pole): ");            String ruch = scanner.nextLine();
            try {
                ...
                if (czyMaPustePole(plansza, wier, pol)) {
                    plansza.getPlansza().get(wier).getWiersz().get(pol).setSymbol(Symbol.X);
                    return;
                }
            } catch (Exception e) {
